...
public class PluginResult {
    ...
    public PluginResult getMultipartMessage(int i4) {
        return this.multipartMessages.get(i4);
    }
    ...
    public void setKeepCallback(boolean z3) {
        this.keepCallback = z3;
    }
    ...
    public String toCallbackString(String str) {
        ...
        if (i4 == status2.ordinal() && this.keepCallback) {
            return null;
        }
        if (this.status == Status.OK.ordinal() || this.status == status2.ordinal()) {
    ...
    public PluginResult(Status status2, int i4) {
        this.keepCallback = false;
        this.status = status2.ordinal();
        this.messageType = 3;
        this.encodedMessage = "" + i4;
    }

    public PluginResult(Status status2, float f4) {
        this.keepCallback = false;
        this.status = status2.ordinal();
        this.messageType = 3;
        this.encodedMessage = "" + f4;
    }

    public PluginResult(Status status2, boolean z3) {
        this.keepCallback = false;
        this.status = status2.ordinal();
        this.messageType = 4;
        this.encodedMessage = Boolean.toString(z3);
    }
    ...
    public PluginResult(Status status2, byte[] bArr, boolean z3) {
        this.keepCallback = false;
        this.status = status2.ordinal();
        this.messageType = z3 ? 7 : 6;
